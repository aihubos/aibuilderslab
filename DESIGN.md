...
- 참여 채널: 별도 최상단 띠를 없애고 개인 카카오톡, 카카오톡 단체방, 당근 모임을 주요 내비게이션 안에 통합한다. 카카오는 사용자가 제공한 실제 아이콘 옆에 `개인`, `단체방`만 표시한다. 당근은 사용자가 제공한 실제 워드마크를 사용한다. 모바일에서는 햄버거 메뉴 안에 배치하고 네이버 블로그는 넣지 않는다.
